...
let qmdurchfuehrer = prompt("Bitte User ID vom QM-Durchf�hrer Eingeben", "XXXX");
...
async function addQMassnahmen() {
    ...
    for (let numberOfQMassnahmenArr = 0; numberOfQMassnahmenArr < qMassnahmenArr.length; numberOfQMassnahmenArr++) {
        ...
        for (let numberOfCSVArray = 0; numberOfCSVArray < CSVArray.length; numberOfCSVArray++) {
            let skippedElements = 0;
            if (CSVArray[numberOfCSVArray]["Q-Ma�nahme"] !== qMassnahmenArr[numberOfQMassnahmenArr]) {
                skippedElements++;
                continue;
            }
...
alert(`Das Skript ist beendet. Bitte geben sie den Change unter "Release ID (HPSM) / Sammler ID (ServiceNow)" oder "Normal Change ID" ein und weisen sie die Q-Massnahme einem Benutzer zu.`); 
}  